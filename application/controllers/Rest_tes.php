...
class Rest_tes extends REST_Controller
{
...
#---MASTER
    ...
    public function ranking_partner_post()
    {
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0 && ($f_active == 'true' || $f_active == 'false') && $rank_code != '' && $rank_name != '') {
    ...
    public function industry_type_post()
    {
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0 && ($f_active == 'true' || $f_active == 'false') && $industry_type_code != '' && $industry_type_name != '') {
    ...
    public function line_business_post()
    {
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0 && ($f_active == 'true' || $f_active == 'false') && $line_business_code != '' && $line_business_name != '') {
    ...
    public function coa_post()
    {
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0 && ($f_active == 'true' || $f_active == 'false') && $code_account != '' && $group_coa != '') {
    ...
    public function user_policy_post()
    {
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0 && ($f_active == 'true' || $f_active == 'false') && $username != '' && $username_rolename != '') {
            $this->db->select("username");
...
#//CASH BANK
    ...
    public function terima_cekgiro_post()
    {
        ...
        $realization_date   = $this->post('realization_date');
    ...
    public function setor_giro_post()
    {
        ...
        $i_company          = $this->post('i_company');
        $ou_code            = $this->post('ou_code');
        $depositor_no       = $this->post('depositor_no');
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0 && $ou_code != '' && $depositor_no != '') {
            $this->db->select("ou_code, depositor_no");
            $this->db->from("tbl_setor_giro");
            $this->db->where("ou_code", $ou_code);
            $this->db->where("depositor_no", $depositor_no);
            $this->db->where("i_company", $i_company);
            $cek_query = $this->db->get();
            ...
            if ($cek_query->num_rows() > 0) {
                ...
                $message = "Data setor giro " . $depositor_no . " Berhasil di update";
            } else {               
                $data = array(
                                'i_company'         => $i_company,
                                'ou_code'           => $ou_code,
                                'depositor_no'      => $depositor_no,
                                'depositor_date'    => $depositor_date,
                                'bank_code'         => $bank_code,
                                'bank_name'         => $bank_name,
                                'partner_code'      => $partner_code,
                ...
                $message = "Data setor giro " . $depositor_no . " Berhasil di input";
    ...
    public function request_cashadvance_post()
    {
        ...
        $i_company                  = $this->post('i_company');
        $ou_code                    = $this->post('ou_code');
        $request_cahsadvance_no     = $this->post('request_cahsadvance_no');
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0 && $ou_code != '' && $request_cahsadvance_no != '') {
            $this->db->select("ou_code, request_cahsadvance_no");
            $this->db->from("tbl_request_cashadvance");
            $this->db->where("ou_code", $ou_code);
            $this->db->where("request_cahsadvance_no", $request_cahsadvance_no);
            $this->db->where("i_company", $i_company);
            $cek_query = $this->db->get();
            ...
            if ($cek_query->num_rows() > 0) {
                ...
                $message = "Data request cash advance " . $request_cahsadvance_no . " Berhasil di update";
            } else {  
                $data = array(
                                'i_company'                 => $i_company,
                                'ou_code'                   => $ou_code,
                                'request_cahsadvance_no'    => $request_cahsadvance_no, 
                                'request_cashadvance_date'  => $request_cashadvance_date,
                                'cash_type'                 => $cash_type,
                                'employee_code'             => $employee_code,
                                'employee_name'             => $employee_name,
                ...
                $message = "Data request cash advance " . $request_cahsadvance_no . " Berhasil di input";
            ...
            if ($this->post('item')) {
                foreach ($this->post('item') as $row) {
                    $data = array(
                                    ...
                                    'cashbank_amount'       => $row['cashbank_amount'],
                                    'amount'                => $row['amount'],
                                    'remark'                => $row['remark'],
                    );
...
#--PIUTANG
    public function invoice_ar_post()
    {
        ...
        $status_faktur_invoice  = $this->post('status_faktur_invoice');
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0 && $ou_code != '' && $invoice_no != '') {
            ...
            if ($cek_query->num_rows() > 0) {
                $data = array( 
                                ...
                                'status_faktur_invoice'     => $status_faktur_invoice,
                                'modifiedat'                => $datenow,
                );   
                $this->db->where("ou_code", $ou_code);
                $this->db->where("invoice_no", $invoice_no);
            ...
            if ($this->post('item')) {
                foreach ($this->post('item') as $row) {
                    $data = array(
                                    ...
                                    'pelunasan_type'        => $row['pelunasan_type'],
                                    'invoice_no'            => $row['invoice_no'],
                                    'invoice_date'          => $row['invoice_date'],
...
#---IVENTORY    
            ...
            if ($this->post('item')) {
                foreach ($this->post('item') as $row) {
                    $data = array(
                                    ...
                                    'convert_sales'     => $row['convert_sales']
                    );
    ...
    public function dkb_post()
    {
        ...
        $do_no             = $this->post('do_no');
        $do_packinglist_no = $this->post('do_packinglist_no');
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0  && $ou_code != '' && $dkb_no != '') {
            ...
            if ($cek_dlv->num_rows() > 0) {
                $data = array(
                                ...
                                'dkb_date'          => $dkb_date,
                                'do_no'             => $do_no,
                                'do_packinglist_no' => $do_packinglist_no,
    ...
    public function do_receipt_post()
    {
        ...
        if ($action == 'create' && $cek_company->num_rows() > 0  && $ou_code != '' && $receipt_no != '') {
            $this->db->select("do_no, ou_code, receipt_no");
            ...
            if ($cek_dlv->num_rows() > 0) {
                ...
                $message = "Data DO Receipt " .$receipt_no. " Berhasil di update";
            } else {
                $data = array(
                                'i_company'       => $i_company,
                                'ou_code'         => $ou_code,
                ...
                $message = "Data DO Receipt " . $receipt_no . " Berhasil di input";
                $this->db->insert('tbl_do_receipt', $data);
            }
            $query = $this->db->query("DELETE FROM tbl_do_receipt_item where receipt_no = '$receipt_no' and i_company = '$i_company'");
            if ($this->post('item')) {
                foreach ($this->post('item') as $row) {
                    $data = array(
                                    'i_company'         => $row['i_company'],
            ...
            if ($this->post('item')) {
                foreach ($this->post('item') as $row) {
                    $data = array(
                                    ...
                                    'qty_do_invoice'    => $row['qty_do_invoice'],
                                    'product_status'    => $row['product_status'],
                                    'remark'            => $row['remark'],
                    );
