...
class Api_toko extends REST_Controller
{
    ...
    public function login_post() {
        ...
        $password = encrypt_password($this->post('password'));
        ...
        if ($check_login->num_rows() > 0) {
            if ($check_login->row()->e_password == $password) {
                $this->db->query("
                    INSERT INTO public.tbl_aktivitas (i_company, username, aktivitas)
                    select distinct id_company, '$username', 'Login' from tbl_user_toko_item  where username = '$username';
                ", FALSE);
    ...
    public function main_header_post() {
        ...
        if ($data->num_rows() > 0) {
            ...
            $this->db->query("
                    INSERT INTO public.tbl_aktivitas (i_company, username, aktivitas)
                    select distinct id_company, '$username', 'Membuka Halaman Utama' from tbl_user_toko_item  where username = '$username';
                ", FALSE);
    ...
    public function bank_post() {
        ...
        if ($data->num_rows() > 0) {
            ...
            $data3 = $this->db->query("
                select e_image_path, e_bank_name, i_norek, 'Atas Nama : ' || e_norek_name as e_norek_name from tbl_bank 
            ...
            $key = 0;
            if ($data3->num_rows() > 0) {
                foreach ($data3->result() as $rew) {
                    $listbank[$key]['e_image_path'] = $rew->e_image_path;
                    $listbank[$key]['e_bank_name'] = $rew->e_bank_name;
                    $listbank[$key]['i_norek'] = $rew->i_norek;
                    $listbank[$key]['e_norek_name'] = $rew->e_norek_name;
                    $key++;
                }
            } 
            ...
            $this->db->query("
                    INSERT INTO public.tbl_aktivitas (i_company, username, aktivitas)
                    select distinct id_company, '$username', 'Membuka Halaman Pembayaran' from tbl_user_toko_item  where username = '$username';
                ", FALSE);

           $this->response([
                'status' => true,  
                'v_sisa' => number_format($sisa,0),           
                'listbank' => $listbank,  
                'listnota' => $list,  
            ], REST_Controller::HTTP_OK);
    ...
    public function contact_post() {
        ...
        $username = $this->post('username');
        $data3 = $this->db->query("
            select e_image_path, e_name, e_link, com_package from tbl_contact_us where f_active = true and id_company in (
                 select distinct b.id_company from tbl_user_toko a 
                 inner join tbl_user_toko_item b on (a.username = b.username) 
                 where a.username ilike '$username' limit 1
            ) order by createdat asc;
        ...
        if ($data3->num_rows() > 0) {
            foreach ($data3->result() as $rew) {
                $listcontact[$key]['e_image_path'] = $rew->e_image_path;
                $listcontact[$key]['e_name'] = $rew->e_name;
                $listcontact[$key]['e_link'] = $rew->e_link; 
                $listcontact[$key]['com_package'] = $rew->com_package;
                $key++;
            }
        } 
        ...
        $this->db->query("
                    INSERT INTO public.tbl_aktivitas (i_company, username, aktivitas)
                    select distinct id_company, '$username', 'Membuka Halaman Kontak' from tbl_user_toko_item  where username = '$username';
                ", FALSE);
    ...
    public function update_password_post() {
        ...
        // $data3 = $this->db->query("
        //     select username , e_password from tbl_user_toko where f_active = true and createdat >= '2022-04-14 16:16:19'::timestamp ;
        // ", FALSE);
        ...
        //        $password = encrypt_password($rew->e_password);

        //       $this->db->query("
        //         UPDATE public.tbl_user_toko SET e_password='$password' WHERE username='$username';
        //       ", FALSE);
