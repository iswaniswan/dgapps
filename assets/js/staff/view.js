var Staff = (function () {
          ...
          for (let x = 0; x < result.data.length; x++) {
            function createInfoWindowContent() {
              ...
              return [
                ...
                "LatLng: " +
                  result.data[x].latitude_checkin +
                  " , " +
                  result.data[x].longitude_checkin,
              ].join("<br>");
            }
          ...
          directionsService.route(
            {
              ...
              destination: {
                lat: parseFloat(
                  result.data[result.data.length - 1].latitude_checkin
                ),
                lng: parseFloat(
                  result.data[result.data.length - 1].longitude_checkin
                ),
              },
              waypoints: waypoints,
      ...
      success: function (result) {
        ...
        function initialize() {
          ...
          var map_marker_simple_element =
            document.getElementById("map-tracking");
          ...
          for (let x = 0; x < result.data.length; x++) {
            ...
            if (x == 0) {
              markers.push(
                new google.maps.Marker({
                  position: new google.maps.LatLng(
                    parseFloat(result.data[x].latitude),
                    parseFloat(result.data[x].longitude)
                  ),
                  map: map,
                  draggable: false,
                  label: String("1"),
  ...
  var _componentFullCalendarStyling = function () {
    ...
    // ------------------------------
    // Define element
    var calendarEventColorsElement = document.querySelector(
      ".fullcalendar-event-colors"
    );

    // Initialize
    if (calendarEventColorsElement) {
      var calendarEventColorsInit = new FullCalendar.Calendar(
        calendarEventColorsElement,
        {
...
function basicPopup(url) {
  popupWindow = window.open(
    url,
    "popUpWindow",
    "height="+screen.availHeight+",width="+screen.availWidth+",left=100,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes"
  );
}
